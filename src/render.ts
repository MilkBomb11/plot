...
function render (
    ...
    function renderEquation(equation: Op.T[], canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
        ...
        function drawQuad(sx: number, sy: number, size: number) {
            ...
            const result = evaluate(equation, xInterval, yInterval);
    ...
    drawAxes();
    for (const equation of equations) { renderEquation(equation, canvas, ctx); }
}
